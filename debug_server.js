...
app.use(cors({
    origin: [
        'https://ritians-transport.web.app',
        'https://ritians-transport.firebaseapp.com',
        'http://localhost:5173',
        'http://localhost:3000',
        'http://localhost:3001'
    ],
...
app.use(express.static(path.join(process.cwd(), 'dist'))); // Serve built frontend files
...
const authorizeRole = (role) => {
    return (req, res, next) => {
        if (req.user.role !== role) {
            return res.status(403).json({ error: `Access denied. ${role} permissions required.` });
        }
        next();
    };
};
...
app.get('/api/admin/check', authenticateToken, authorizeRole('admin'), (req, res) => {
    res.json({ status: 'Admin SDK initialized successfully', projectId: serviceAccount.project_id });
});
...
// Update Route Stops
app.post('/api/routes/:routeNo/stops', authenticateToken, authorizeRole('admin'), async (req, res) => {
    try {
...
// Delete Route
app.delete('/api/routes/:routeNo', authenticateToken, authorizeRole('admin'), async (req, res) => {
    try {
        const { routeNo } = req.params;
...
app.get('/api/routes', async (req, res) => {
    try {
        ...
        // Fallback to hardcoded data if DB is empty (implicit seeding logic or just safety)
        if (routesData.length > 0) {
            res.json({ routes: routesData, routeStops: routeStopsData });
        } else {
            console.log("⚠️ Firestore is empty. Returning hardcoded local data.");
...
app.get('/api/locations', async (req, res) => {
    try {
        const ref = rtdb.ref('locations'); // Assuming 'locations' node exists
...
app.post('/api/admin/seed', async (req, res) => {
    try {
        ...
        // Seed Stops
        const stopEntries = Object.entries(routeStops);
        console.log(`Seeding ${stopEntries.length} route stops...`);
        ...
        console.log("Seeding default authority accounts...");
        ...
        // Add requested user for Daresh (Admin role)
        const dAdminRef = db.collection('authority').doc('daresh_admin');
        const hDareshPw = await bcrypt.hash("dare1911", 10);
        batch.set(dAdminRef, {
            email: "daresh928@gmail.com",
            password: hDareshPw,
            role: "admin",
        ...
        if (operationCount > 500) {
            throw new Error(`Too many operations (${operationCount}). Firestore batch limit is 500.`);
        }

        await batch.commit();
        console.log("✅ Database seeded successfully!");
        res.json({ message: "Database seeded successfully with routes, stops, and staff accounts!" });
    } catch (error) {
...
// -------- Authority & Login Endpoints --------

// Seed Authority Data (Admin & Driver)
app.post('/api/admin/seed-authority', async (req, res) => {
    try {
        const { masterKey } = req.body;
        if (masterKey !== process.env.MASTER_KEY) {
            return res.status(403).json({ error: "Unauthorized: Invalid master key." });
        ...
        res.json({ message: "Authority database seeded! Admin & Driver credentials created." });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});
...
app.post('/api/register', async (req, res) => {
    try {
        const { role, name, email, password, driverId, pin, masterKey } = req.body;

        // Security check
        if (masterKey !== process.env.MASTER_KEY) {
            return res.status(403).json({ error: "Unauthorized: Invalid registration secret." });
        }
...
app.post('/api/login/admin', async (req, res) => {
    try {
        ...
        console.log(`[LOGIN] Admin attempt: email=${email}`);
        ...
        if (snapshot.empty) {
            console.log(`[LOGIN] Admin FAILED: user not found: ${email}`);
        ...
        // Check if password in DB is hashed or plain text
        ...
        if (!isMatch) {
            console.log(`[LOGIN] Admin FAILED: password mismatch for ${email}`);
            return res.status(401).json({ error: "Invalid email or password" });
        }

        console.log(`[LOGIN] Admin SUCCESS: ${email}`);
...
// Driver Login (supports email+password OR driverId+pin)
app.post('/api/login/driver', async (req, res) => {
    try {
        ...
        if (email && password) {
            ...
            return res.json({ success: true, user: { name: user.name, email: user.email, role: user.role, driverId: user.driverId }, token });

        } else if (driverId && pin) {
            // Legacy Driver ID + PIN login
            snapshot = await db.collection('authority')
                .where('role', '==', 'driver')
...
// -------- Midnight Reset Logic (Daily) --------
// Feedback Submission Endpoint
app.post('/api/feedback', async (req, res) => {
    try {
        const { type, tags, message } = req.body;
        if (!type) return res.status(400).json({ error: "Feedback type is required" });
...
// Driver Update Location & Availability
app.post('/api/driver/update', authenticateToken, authorizeRole('driver'), async (req, res) => {
    try {
        const { routeNo, location, availability, driverId } = req.body;
        if (!routeNo || !location) {
            return res.status(400).json({ error: "Route No and Location are required" });
        }

        const ref = rtdb.ref(`locations/${routeNo}`);
        await ref.set({
            location,
            availability: availability || "Available",
            updatedBy: driverId || req.user.driverId || "Unknown",
            timestamp: Date.now()
        });
...
// Admin Manual Reset
app.post('/api/admin/reset-daily', authenticateToken, authorizeRole('admin'), async (req, res) => {
    try {
        await rtdb.ref('locations').set({});
        console.log("✅ Manual reset triggered by Admin.");
        res.json({ success: true, message: "Daily data reset successfully." });
    } catch (error) {
        res.status(500).json({ error: error.message });
    }
});
...
// -------- Midnight Reset Logic (Daily) --------
let lastResetDate = new Date().toLocaleDateString('en-GB', { timeZone: 'Asia/Kolkata' });

function checkAndResetDaily() {
    const todayIST = new Date().toLocaleDateString('en-GB', { timeZone: 'Asia/Kolkata' });

    if (todayIST !== lastResetDate) {
        console.log(`🕛 New day detected (${todayIST}): Resetting active parking locations...`);
        rtdb.ref('locations').set({}).then(() => {
            console.log("✅ Locations reset successfully for the new day.");
...
setInterval(checkAndResetDaily, 30 * 1000);
